...
- Usando os mesmos dados, selecione apenas as colunas que começando com _nome_ ou terminando com _eleitoral_:
...
tab3 <- congressistas %>%
  filter(casa == "senado") %>%
  mutate(nome =
           paste(nome_eleitoral, " ", "(", sg_partido, "/", uf, ")",
                 sep = "")) %>%
  select(id, nome)
...
- Gere uma nova coluna no banco original chamada de _status_ para indicar se o parlamentar é _novato_ ou _reeleito_, selecione a sigla da Casa, o nome político e a variável _status_ que você criou: 
...
- O que isso representa em termos de taxa ou porcentagem?
