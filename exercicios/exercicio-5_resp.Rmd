...
dados_deputados <-  consulta_cand_2018 %>% 
  filter(DS_CARGO == "DEPUTADO FEDERAL") 

dim(consulta_cand_2018)
...
votacao_deputados <-  votacao_candidato_2018 %>% 
  mutate(DS_CARGO = str_to_upper(DS_CARGO)) %>% 
  filter(DS_CARGO == "DEPUTADO FEDERAL") 

dim(votacao_candidato_2018)

dim(votacao_deputados)
```

```{r}
# passo 3:

tab_1 <- votacao_deputados %>% 
  group_by(SG_UF, NM_URNA_CANDIDATO, NR_CANDIDATO, SG_PARTIDO, SQ_CANDIDATO) %>%
...
```{r}
# mesmo resultado, mas sem mensagem chata:

tab_1a <- votacao_deputados %>% 
  group_by(SG_UF, NM_URNA_CANDIDATO, NR_CANDIDATO, SG_PARTIDO, SQ_CANDIDATO) %>%
  summarise(VOTOS_TOTAIS = sum(QT_VOTOS_NOMINAIS), .groups = 'drop')
...
resultado_join <- left_join(dados_deputados %>%
                               select(NR_CPF_CANDIDATO, SQ_CANDIDATO) %>%
                               distinct(NR_CPF_CANDIDATO, SQ_CANDIDATO), 
                             tab_1, 
...
```{r}
# by = c("a" = "b", "c" = "d") ligará x$a ao y$b e x$c ao y$d.
# passo 5:

resultado  <- left_join(congressistas, resultado_join %>% 
                          select(SQ_CANDIDATO, NR_CPF_CANDIDATO, VOTOS_TOTAIS) ,
            by = c("cpf" = "NR_CPF_CANDIDATO"))
...
resultado %>% 
  select(id, casa, cpf, nome_eleitoral, SQ_CANDIDATO, VOTOS_TOTAIS)
```
...
votacao_deputados %>% 
  group_by(SG_UF, NM_URNA_CANDIDATO, NR_CANDIDATO, SG_PARTIDO, SQ_CANDIDATO) %>%
  summarise(VOTOS_TOTAIS = sum(QT_VOTOS_NOMINAIS, na.rm=TRUE), .groups = "keep") %>% 
  group_by(SG_UF) %>%
  mutate(VOTOS_UF =  sum(VOTOS_TOTAIS, na.rm = TRUE)) %>%
  mutate(PERC = 100*(VOTOS_TOTAIS/VOTOS_UF) ) %>%
  arrange(SG_UF, desc(PERC))
...
votacao_deputados %>% 
  ...
  mutate(VOTOS_PARTIDO =  sum(VOTOS_TOTAIS, na.rm = TRUE)) %>%
  mutate(PERC = 100*(VOTOS_TOTAIS/VOTOS_PARTIDO) ) %>%
  arrange(SG_UF, desc(PERC))
...
```{r}
cands_sudeste <- consulta_cand_2018 %>% 
  mutate(REGIAO = ifelse(SG_UF %in% c("SP", "RJ", "ES", "MG"), "Sudeste", "Outra" )) %>%
...
```{r}
cands_sul <- consulta_cand_2018 %>% 
  filter(SG_UF == "PR" | SG_UF == "SC" | SG_UF == "RS") %>% 
  filter(DS_CARGO == "DEPUTADO FEDERAL")
...
```{r}
dados_sudeste_sul <-bind_rows(cands_sudeste, cands_sul)

dim(dados_sudeste_sul)
```
