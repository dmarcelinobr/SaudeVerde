...
for(i in lista.candidatos){
  ...
  eval(parse(text=paste0("dados_prov = read_delim('",i,"',
                         ...
                         trim_ws = TRUE)")))
  consulta_cand_2018 = rbind(consulta_cand_2018, dados_prov)
}
...
#criando uma lista para pegar somente os documentos de votação
lista.candidatos <- grep(pattern="votacao_candidato_munzona_2018_", lista.arquivos, value=TRUE)

# Excluindo Brasil
lista.candidatos = lista.candidatos[!lista.candidatos ==  "votacao_candidato_munzona_2018_BRASIL.csv"]
...
for(i in lista.candidatos){
  ...
  eval(parse(text=paste0("dados_prov = read_delim('",i,"',
                         ...
                         locale = locale(encoding = 'WINDOWS-1252'),
                         trim_ws = TRUE)")))
 votacao_candidato_2018 = rbind(votacao_candidato_2018, dados_prov)
}
...
-  Vamos criar um novo banco de dados, mantendo apenas os candidatos a deputado federal:
...
- Vamos somar os votos do banco de dados em seguida vamos plugar essa informação na tabela de deputados:
